...
Write-Host "Frontend (Firebase): $(if($firebaseSuccess){'✅ Desplegado'}else{'⚠️ Con advertencias'})" -ForegroundColor $(if($firebaseSuccess){[System.ConsoleColor]::Green}else{[System.ConsoleColor]::Yellow})
Write-Host "Backend (Railway): $(if($railwaySuccess){'✅ Desplegado'}else{'⚠️ Con advertencias'})" -ForegroundColor $(if($railwaySuccess){[System.ConsoleColor]::Green}else{[System.ConsoleColor]::Yellow})
Write-Host "API: $(if($apiSuccess){'✅ Funcionando'}else{'❌ Con problemas'})" -ForegroundColor $(if($apiSuccess){[System.ConsoleColor]::Green}else{[System.ConsoleColor]::Red})
Write-Host ""
